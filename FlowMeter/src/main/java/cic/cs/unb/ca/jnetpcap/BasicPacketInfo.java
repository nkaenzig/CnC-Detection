...
/* ******************************************** */    
    ...
    public Integer IntOrExt(String[] intSubnets) {

		boolean isInSubnet = false;
		SubnetUtils utils;
		String dstIp = getDestinationIP();

		for (int i = 0; i < intSubnets.length; i++){
			utils = new SubnetUtils(intSubnets[i]);
			isInSubnet = utils.getInfo().isInRange(dstIp);

			if(isInSubnet){
//				System.out.println(dstIp);
//				System.out.println(intSubnets[i]);
				break;
			}
		}

		return isInSubnet ? 0 : 1;
	}
	...
	public void setTCPWindow(int TCPWindow){ this.TCPWindow = TCPWindow; }
}
