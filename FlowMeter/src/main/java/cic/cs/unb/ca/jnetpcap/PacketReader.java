...
public class PacketReader {
	...
	public BasicPacketInfo nextPacket(){
		 ...
		 try{
		     int read_status = pcapReader.nextEx(hdr,buf);
			 if(read_status == Pcap.NEXT_EX_OK){
			     ...
			     if(read_status==Pcap.NEXT_EX_NOT_OK){
                     throw new PcapClosedException("NEXT_EX_NOT_OK: " + pcapReader.getErr());
                 }else if(read_status==Pcap.NEXT_EX_TIMEDOUT){
                     throw new PcapClosedException("NEXT_EX_TIMEDOUT: " + pcapReader.getErr());
                 }else if(read_status==Pcap.NEXT_EX_EOF) {
                     throw new PcapClosedException("Reached end of file.: " + pcapReader.getErr());
//					 throw new Exception("just a test");
                 }else{
                     throw new PcapClosedException("Unknown pcap parsing error");
                 }
			 }
