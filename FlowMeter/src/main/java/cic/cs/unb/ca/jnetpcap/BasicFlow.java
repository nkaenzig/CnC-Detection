...
public class BasicFlow {
    ...
    private 	int dstIntExt = -1; // 0 for internal IP, 1 for external IP
    

	public BasicFlow(boolean isBidirectional,BasicPacketInfo packet, byte[] flowSrc, byte[] flowDst, int flowSrcPort, int flowDstPort, String[] intSubnets) {
		super();
		this.initParameters();
		...
		if(intSubnets != null){
			this.dstIntExt = packet.IntOrExt(intSubnets);
		}
	}
    
	public BasicFlow(boolean isBidirectional, BasicPacketInfo packet, String[] intSubnets) {
		super();
		this.initParameters();
		this.isBidirectional = isBidirectional;
		this.firstPacket(packet);
    ...
    public String dumpFlowBasedFeaturesEx() {
		...
		if((double)flowDuration/1000000L > 0){
			...
			dump.append(0).append(separator);//21
			dump.append(0).append(separator);//22
		}
    	...
    	dump.append(Integer.toString(dstIntExt)).append(separator);
